...
# SPDX-PackageDownloadLocation: https://forge.colobox.com/rfinnie/2ping
...
class TwoPing:
    def __init__(self, args):
        ...
        self.auth_digest_index = {v[2].lower(): k for k, v in packets.OpcodeHMAC().digest_map.items()}[self.args.auth_digest]
        self.encrypt_method_index = {v[0].lower(): k for k, v in packets.OpcodeEncrypted().method_map.items()}[
    ...
    def sock_classes_active(self):
        ...
        return [
            ...
            if not sock_class.shutdown_time or (sock_class.shutdown_time and sock_class.shutdown_time > now)
        ]
    ...
    def process_incoming_packet(self, sock_class):
        ...
        if self.args.encrypt:
            if packets.OpcodeEncrypted.id not in packet_in.opcodes:
                sock_class.errors_received += 1
                self.logger.error(_("Encryption required but not provided by {address}").format(address=print_address))
                return
            if packet_in.opcodes[packets.OpcodeEncrypted.id].method_index != self.encrypt_method_index:
                sock_class.errors_received += 1
                self.logger.error(
            ...
            if encrypted_packet_in.opcodes[packets.OpcodeEncrypted.id].session != peer_state.encrypted_session_id:
                ...
                self.logger.error(
                    _("Encryption session mismatch from {address} (expected {expected}, got {got})").format(
                        ...
                        got=encrypted_packet_in.opcodes[packets.OpcodeEncrypted.id].session,
                    )
                )
                return
        ...
        if self.args.auth:
            if packets.OpcodeHMAC.id not in packet_in.opcodes:
                sock_class.errors_received += 1
                self.logger.error(_("Auth required but not provided by {address}").format(address=print_address))
                return
            if packet_in.opcodes[packets.OpcodeHMAC.id].digest_index != self.auth_digest_index:
                sock_class.errors_received += 1
                self.logger.error(
        ...
        if packets.OpcodeInReplyTo.id in packet_in.opcodes:
            ...
            if replied_message_id in peer_state.sent_messages:
                ...
                if self.args.subtract_peer_host_latency and packets.OpcodeHostLatency.id in packet_in.opcodes:
                ...
                if self.args.quiet or self.args.flood:
                    ...
                    if packets.OpcodeRTTEnclosed.id in packet_in.opcodes:
                        self.logger.info(
                            _(
                                ("{bytes} bytes from {address}: ping_seq={seq} time={ms:0.03f} ms " "peertime={peerms:0.03f} ms")
                            ).format(
                                bytes=len(data),
                                address=print_address,
                                seq=ping_position,
                    ...
                    if (packets.OpcodeExtended.id in packet_in.opcodes) and (
                        ...
                        self.logger.info("  " + _("Peer notice: {notice}").format(notice=notice))
        ...
        if packets.OpcodeReplyRequested.id in packet_in.opcodes:
            ...
            if self.args.encrypt:
                ...
                encrypted_packet.opcodes[packets.OpcodeEncrypted.id].method_index = self.encrypt_method_index
    ...
    def sock_sendto(self, sock_class, data, address=None):
        ...
        if self.args.packet_loss and (random.random() < (self.args.packet_loss.out_pct / 100.0)):
            return
        # Send the packet.
        try:
            if address:
    ...
    def sock_recvfrom(self, sock_class):
        ...
        # Simulate random packet loss.
        if self.args.packet_loss and (random.random() < (self.args.packet_loss.in_pct / 100.0)):
            return
    ...
    def check_investigations(self, peer_state, packet_check):
        ...
        for ping_seq in sorted(found):
            ...
            self.logger.error(_(loss_message).format(loss_type=loss_type, address=address, seq=ping_seq))
    ...
    def gather_systemd_socks(self):
        ...
        systemd_sock_classes = [sock_class for sock_class in self.sock_classes if sock_class.is_systemd]
        ...
        for fd in systemd_daemon.listen_fds():
            for family in (socket.AF_INET6, socket.AF_INET):
                if not systemd_daemon.is_socket_inet(fd, family=family, type=socket.SOCK_DGRAM):
                    continue
    ...
    def setup_interface_address(
        ...
        sock_classes = []
        for addrinfo in socket.getaddrinfo(interface_address, port, family, type, proto):
            if (addrinfo[0] == socket.AF_INET6) and (not self.args.ipv4) and self.has_ipv6:
                pass
    ...
    def get_system_addresses(self):
        ...
        for iface in netifaces.interfaces():
            ...
            if (self.args.ipv4 or (not self.args.ipv4 and not self.args.ipv6)) and (netifaces.AF_INET in iface_addrs):
                addrs.update([(f["addr"], netifaces.AF_INET) for f in iface_addrs[netifaces.AF_INET] if "addr" in f])
            if (
                self.has_ipv6
                and (self.args.ipv6 or (not self.args.ipv4 and not self.args.ipv6))
                and (netifaces.AF_INET6 in iface_addrs)
            ):
                addrs.update([(f["addr"], netifaces.AF_INET6) for f in iface_addrs[netifaces.AF_INET6] if "addr" in f])
    ...
    def set_high_port(self):
        ...
        for attempt in range(100):
            ...
            addrinfo = socket.getaddrinfo(interface_address, port, family, socket.SOCK_DGRAM, socket.IPPROTO_UDP)[0]
            ...
            sock.close()
            if attempt > 0:
                self.logger.warning("It took {} attempts to get an unused port: {}".format(attempt + 1, caught_errors))
    ...
    def get_interface_addresses(self):
        if self.args.interface_address:
            ...
            interface_addresses = [(addr, socket.AF_UNSPEC) for addr in self.args.interface_address]
    ...
    def setup_sockets_loopback(self):
        ...
        if [sock_class for sock_class in self.sock_classes_open if sock_class.is_loopback and sock_class.is_client]:
            return

        for i in range(self.args.loopback_pairs):
            sock_client, sock_listener = socket.socketpair(socket.AF_UNIX, socket.SOCK_DGRAM)
    ...
    def setup_sockets_listener(self):
        ...
        if [sock_class for sock_class in self.sock_classes_open if sock_class.is_systemd]:
            return
        ...
        listener_sock_classes = [
            sock_class for sock_class in self.sock_classes_open if sock_class.is_inet and sock_class.is_listener
        ]
        to_close = []
    ...
    def setup_sockets_client(self):
        # Client socket setup is only done once.
        if [sock_class for sock_class in self.sock_classes_open if sock_class.is_inet and sock_class.is_client]:
            return
    ...
    def send_new_ping(self, sock_class, peer_address=None):
        ...
        packet_out_examine.load(dump_out)
        if self.args.verbose:
            self.logger.info("SEND{}: {}".format((" (encrypted)" if self.args.encrypt else ""), packet_out_examine))
    ...
    def get_nagios_stats(self):
        n = self.args.nagios
        sock_class = [sock_class for sock_class in self.sock_classes if sock_class.is_client][0]
        ...
        nagios_stats_text = _("{result} 2PING - Packet loss = {loss}%, RTA = {avg:0.03f} ms").format(
            result=nagios_result_text, loss=int(lost_pct), avg=rtt_avg
        ) + ("|rta={avg:0.06f}ms;{avgwarn:0.06f};{avgcrit:0.06f};0.000000 pl={loss}%;{losswarn};{losscrit};0").format(
            avg=rtt_avg,
            loss=int(lost_pct),
            avgwarn=n.warn_rta,
            avgcrit=n.crit_rta,
    ...
    def print_stats_sock(self, sock_class, time_end, short=False):
        ...
        outbound_pct = div0(stats_class.lost_outbound, stats_class.pings_transmitted) * 100
        inbound_pct = div0(stats_class.lost_inbound, stats_class.pings_transmitted) * 100
        ...
        rtt_mdev = math.sqrt(
            div0(stats_class.rtt_total_sq, stats_class.rtt_count) - (div0(stats_class.rtt_total, stats_class.rtt_count) ** 2)
        )
        socket_address = sock_class.address
        if sock_class.client_host:
            if sock_class.client_host[3]:
                hostname = "{} ({})".format(sock_class.client_host[3], sock_class.client_host[4][0])
            else:
        ...
        if short:
            ...
            self.logger.info("")
            self.logger.info("--- {} ---".format(_("{hostname} 2ping statistics").format(hostname=hostname)))
            self.logger.info(
                _pl(
            ...
            self.logger.info(
                _("rtt min/avg/ewma/max/mdev = {min:0.03f}/{avg:0.03f}/{ewma:0.03f}/{max:0.03f}/{mdev:0.03f} ms").format(
                    min=stats_class.rtt_min,
                    avg=rtt_avg,
                    ewma=rtt_ewma,
                    max=stats_class.rtt_max,
    ...
    def run(self):
        ...
        for sock_class in self.sock_classes_active:
            ...
            if sock_class.client_host:
                if sock_class.client_host[3]:
                    host_display = "{} ({})".format(sock_class.client_host[3], sock_class.client_host[4][0])
                else:
    ...
    def base_packet(self):
        ...
        if self.args.send_random:
            random_data = bytes([random.randint(0, 255) for x in range(self.args.send_random)])
            ...
            packet_out.opcodes[packets.OpcodeExtended.id].segments[packets.ExtendedRandom.id].is_os = random_is_systemrandom
        ...
        if self.args.auth:
            ...
            packet_out.opcodes[packets.OpcodeHMAC.id].digest_index = self.auth_digest_index
            ...
            for table_name, max_time in (
                ...
                for table_key_name in tuple(table.keys()):
                    if now > (table[table_key_name][0] + max_time):
                        ...
                        self.logger.debug("Cleanup: Removed {} {} {}".format(peer_tuple, table_name, table_key_name))
    ...
    def get_next_wakeup(self):
        ...
        for sock_class in self.sock_classes_active:
            ...
            if sock_class.shutdown_time > now:
                events.append((sock_class.shutdown_time, "shutdown ({})".format(sock_class)))
        if self.args.stats:
    ...
    def loop_client_send(self):
        ...
        for sock_class in self.sock_classes_active:
            ...
            if self.args.count and (sock_class.pings_transmitted >= self.args.count):
                self.logger.debug("loop_client_send: Setting shutdown time to +{} on {}".format(self.args.interval, sock_class))
    ...
    def loop_poller(self, next_wakeup):
        ...
        for key, mask in self.poller.select(timeout=(0 if timeout < 0 else timeout)):
            ...
            try:
                ...
                self.logger.exception(_("Received unexpected exception (please file a bug report)"))
                if self.args.debug:
                    raise
            ...
            if (
                ...
                self.logger.debug("loop_poller: Setting shutdown time to now on {}".format(sock_class))
                sock_class.shutdown_time = clock()
                sock_class.next_send = 0
    ...
    def loop(self):
        while True:
            ...
            self.logger.debug("Next wakeup: {} ({})".format((next_wakeup - clock()), next_wakeup_reason))
            ...
            if self.args.deadline and now >= (self.time_start + self.args.deadline):
                for sock_class in self.sock_classes_active:
                    self.logger.debug("loop (deadline): Setting shutdown time to now on {}".format(sock_class))
            ...
            if (
                ...
                and len([sock_class for sock_class in self.sock_classes_active if sock_class.is_client]) == 0
            ):
                # Special case for --count and --no-3way on a listener
                for sock_class in [sock_class for sock_class in self.sock_classes_active if sock_class.is_listener]:
                    self.logger.debug("loop (no_3way): Setting shutdown time to now on {}".format(sock_class))
                    sock_class.shutdown_time = clock()
                    sock_class.next_send = 0
